...
<body>
    <div class="birthday">
        ...
        {:else}
            <p class="subtitle">meaning the next birthday party starts in</p>
        {/if}
        {#if bdToday}
                    ...
                    {#each digit.value as number}
                            {#key number}
                                {#if (digit.value[0] == 1 && digit.value[1] == 9 || digit.value[1] == 1 && digit.value[2] == 9) && (number == 1 || number == 9)}
                                    <span in:fly="{{ y: -70, duration: 1000, opacity: 0.1}}" out:fly="{{duration: 0}}">{@html "<span style='color: #fbdd11'>" + number + "</span>"}</span>
                                {:else}
                                    <span in:fly="{{ y: -70, duration: 1000, opacity: 0.1}}" out:fly="{{duration: 0}}">{number}</span>
                                {/if}
                            {/key}
                    {/each}
                    </div>
            ...
            <div class="countdown">
                {#each _digits as digit}
                    ...
                    {#each digit.value as number}
                            {#key number}
                                {#if (digit.value[0] == 1 && digit.value[1] == 9 || digit.value[1] == 1 && digit.value[2] == 9) && (number == 1 || number == 9)}
                                    <span in:fly="{{ y: -70, duration: 1100, opacity: 0.1}}" out:fly="{{duration: 0}}">{@html "<span style='color: #fbdd11'>" + number + "</span>"}</span>
                                {:else}
                                    <span in:fly="{{ y: -70, duration: 1100, opacity: 0.1}}" out:fly="{{duration: 0}}">{number}</span>
                                {/if}
                            {/key}
                    {/each}
                    </div>
